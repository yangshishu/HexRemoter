...
{
	DataRaw[ROLL] = DataRaw[ROLL] * 0.99 + (float)(4095-ADC_ConvertedValue[3])/2 * 0.01;//翻滚
	DataRaw[PITCH] = DataRaw[PITCH] * 0.99 + (float)(ADC_ConvertedValue[2])/2		* 0.01;//倾角
	DataRaw[YAW] = DataRaw[YAW] * 0.99 + (float)ADC_ConvertedValue[1]/2 * 0.01;//航向
	DataRaw[THROTTLE] = DataRaw[THROTTLE] * 0.99 + (float)(4095-ADC_ConvertedValue[0])/2 * 0.01;//油门
...
void ANO_RC::DataCalculate(void)
{
	...
	else if(ano.f.ACCELMODE)//加速度模块方式
	{
